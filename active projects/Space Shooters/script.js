...
const explosion_sounds = ['./sounds/explosion-1.mp3', './sounds/explosion-2.mp3', './sounds/explosion-3.mp3', './sounds/explosion-4.mp3'];
const laser_sound = "./sounds/laser_shot-1.mp3";
const audioPlayer1 = document.getElementById("audioPlayer1");
audioPlayer1.src = laser_sound; audioPlayer1.load();

function playSimultaneous(mp3Files) {
  mp3Files.forEach(mp3File => {
    let i =randomNumber(1,explosion_sounds.length);
    const audio = new Audio(explosion_sounds[i]);
    audio.play();
    console.log(i);
  });
}
...
function CheckCollision(bullet) {
    ...
    enemys.forEach((enemy, index) => {
        ...
        if (
            ...
            playSimultaneous(explosion_sounds);
        }
    });
}
